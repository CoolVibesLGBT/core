...
func NewRouter(
	...
	r.action.Register(constants.CMD_USER_VIEW_PROFILE, handlers.HandleUserViewProfile(userService), middleware.AuthMiddleware(userRepo))
	...
	r.action.Register(constants.CMD_POST_EVENT_RSVP, handlers.HandlePostEventRSVP(postService), middleware.AuthMiddleware(userRepo))
	...
	r.action.Register(constants.CMD_CHAT_MESSAGE_OPEN, handlers.HandleChatMessageOpen(chatService), middleware.AuthMiddleware(userRepo))
