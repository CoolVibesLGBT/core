...
func (r *moderationHandlerRepository) ResolveReport(_ context.Context, input ports.ModerationResolveInput) (*models.Report, error) {
	r.resolve = input
	return &models.Report{ID: input.ReportID, Status: input.Status}, nil
}

func (r *moderationHandlerRepository) SetPostPublished(_ context.Context, postPublicID int64, published bool, _ uuid.UUID, _ string) (*post.Post, error) {
...
func TestModerationFetchAcceptsJSONAndDefaultsPending(t *testing.T) {
	...
	if repo.fetchFilter.Status != models.ReportStatusPending || repo.fetchFilter.ContentableType != models.EngagementContentableTypeUser || repo.fetchFilter.UserPublicID != 88 || repo.fetchFilter.Limit != 7 {
...
func TestModerationResolveAcceptsJSONBoolean(t *testing.T) {
	...
	if repo.resolve.ReportID != reportID || repo.resolve.Status != models.ReportStatusActioned || repo.resolve.PublishPost == nil || *repo.resolve.PublishPost || repo.resolve.ReviewedByID != moderator.ID {
