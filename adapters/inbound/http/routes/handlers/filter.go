...
func ParseFilters(c fiber.Ctx, authUser *models.User) (types.Filter, error) {
	filter := types.Filter{
		AuthUser: actorFromUser(authUser),
		Context:  c.Context(),
	...
	if (filter.Latitude == nil) != (filter.Longitude == nil) {
		return filter, fmt.Errorf("latitude and longitude must be provided together")
	}
	if filter.Latitude != nil {
		if _, err := domainuser.NewCoordinates(*filter.Latitude, *filter.Longitude); err != nil {
			return filter, err
		}
	}
...
func actorFromUser(user *models.User) *types.Actor {
	if user == nil {
		return nil
	}
	return &types.Actor{
