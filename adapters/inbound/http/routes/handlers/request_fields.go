...
const (
	maxReportKindLength        = domainmoderation.MaxKindLength
	maxReportDescriptionLength = domainmoderation.MaxDescriptionLength
)
...
func validateReportFields(kind, description string) error {
	if _, err := domainmoderation.NewKind(kind); err != nil {
		return err
	}
	if _, err := domainmoderation.NewDescription(description); err != nil {
		return err
	}
	return nil
}
