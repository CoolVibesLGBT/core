...
func (r *handlerPostRepo) GetPostsByKind(filters types.Filter) (legacyviews.PostsResult, error) {
...
type handlerEngagementRepo struct {
	...
	reciprocalIntents []domainuser.InteractionStateIntent
	reciprocalState   bool
	recordedView      []models.EngagementKind
	getCalls          int
}

func (r *handlerEngagementRepo) ApplyReciprocalUserInteraction(_ context.Context, _ uuid.UUID, _ uuid.UUID, intent domainuser.InteractionStateIntent) (domainuser.InteractionStateTransition, error) {
	r.reciprocalIntents = append(r.reciprocalIntents, intent)
	transition, err := intent.Transition(r.reciprocalState)
	if err == nil {
		r.reciprocalState = transition.Enabled
	}
...
func TestHandlePostReportRejectsNonPositivePostID(t *testing.T) {
	for _, postID := range []string{"0", "-1"} {
		t.Run(postID, func(t *testing.T) {
			...
			if resp.StatusCode != fiber.StatusBadRequest {
				t.Fatalf("post_id=%s status = %d, want 400", postID, resp.StatusCode)
			}
			if postRepo.reportPostID != 0 {
				t.Fatalf("post_id=%s reached repository", postID)
			}
		})
	}
}
...
func TestHandleUserLikeTogglesUserLike(t *testing.T) {
	...
	if len(engagementRepo.reciprocalIntents) != 1 {
		t.Fatalf("expected one atomic user-like transition, got %d", len(engagementRepo.reciprocalIntents))
	}
	pair, err := engagementRepo.reciprocalIntents[0].EngagementPair()
...
func TestHandleToggleSubscribeReportsDisabledState(t *testing.T) {
	authUser := &models.User{ID: uuid.New(), PublicID: 10}
	targetUser := &models.User{ID: uuid.New(), PublicID: 20}
	userRepo := &handlerUserRepo{byPublicID: map[int64]*models.User{
		authUser.PublicID:   authUser,
	...
	engagementRepo := &handlerEngagementRepo{reciprocalState: true}
	service := usecases.NewUserService(userRepo, &handlerPostRepo{}, &handlerMediaRepo{}, engagementRepo, &handlerNotificationRepo{})

	app := fiber.New()
	app.Post("/", func(c fiber.Ctx) error {
		c.Locals("authenticatedUser", authUser)
		return HandleUserToggleSubscribe(service)(c)
	})
	req := httptest.NewRequest(http.MethodPost, "/", bytes.NewBufferString("subscribed_id=20"))
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	resp, err := app.Test(req)
	if err != nil {
		t.Fatalf("app.Test() error = %v", err)
	...
	if resp.StatusCode != fiber.StatusOK || !bytes.Contains(body, []byte("User unsubscribed successfully")) {
		t.Fatalf("unsubscribe response status=%d body=%s", resp.StatusCode, body)
	}
}
