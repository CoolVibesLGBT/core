...
var strictTipAmountPattern = regexp.MustCompile(`^(?:0|[1-9][0-9]{0,19})(?:\.[0-9]{1,18})?$`)

func parseTipAmount(raw string) (decimal.Decimal, error) {
	value := strings.TrimSpace(raw)
	if !strictTipAmountPattern.MatchString(value) {
		return decimal.Zero, domainwallet.ErrAmountOutOfRange
	}
	amount, err := decimal.NewFromString(value)
	if err != nil {
		return decimal.Zero, domainwallet.ErrInvalidAmount
	}
...
func parsePollChoiceID(raw string) (uuid.UUID, error) {
	value := strings.TrimSpace(raw)
	if parsed, err := uuid.Parse(value); err == nil {
		// Backward compatibility for clients holding a pre-projection response.
		return parsed, nil
	}
	decoded, err := types.DecodeOpaqueID("pc", value)
	if err != nil {
		return uuid.Nil, err
	}
...
func isPollCreationValidationError(err error) bool {
	return errors.Is(err, domainpost.ErrPollQuestionRequired) ||
		errors.Is(err, domainpost.ErrPollOptionsRequired) ||
		errors.Is(err, domainpost.ErrDuplicatePollOption) ||
		errors.Is(err, domainpost.ErrInvalidPollMaximum) ||
		errors.Is(err, domainpost.ErrInvalidPollKind) ||
		errors.Is(err, domainpost.ErrInvalidPollChoiceData)
}
...
func HandleCreate(s *usecases.PostService) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		if err != nil {
			if isPollCreationValidationError(err) {
...
func HandleVote(s *usecases.PostService) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		choiceId, err := parsePollChoiceID(choiceIdStr)
		if err != nil {
		...
		if err != nil {
			if errors.Is(err, domainpost.ErrInvalidPollKind) ||
				errors.Is(err, domainpost.ErrPollSelectionLimit) ||
				errors.Is(err, domainpost.ErrInvalidPollWeight) ||
				errors.Is(err, domainpost.ErrInvalidPollRank) ||
				errors.Is(err, domainpost.ErrDuplicatePollRank) ||
				errors.Is(err, domainpost.ErrInvalidPollChoiceData) {
				return utils.SendErrorWithMessage(c, fiber.StatusBadRequest, constants.ErrVoteFailed, err.Error())
			}
...
func HandlePostTip(s *usecases.PostService) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		amount, err := parseTipAmount(amountStr)
		if err != nil {
		...
		balance, err := s.Tip(c.Context(), postId, user, amount, idempotencyKey)
		if err != nil {
			if errors.Is(err, domainwallet.ErrIdempotencyKeyRequired) ||
				errors.Is(err, domainwallet.ErrInvalidIdempotencyKey) ||
			...
			if errors.Is(err, domainwallet.ErrIdempotencyConflict) {
				return utils.SendErrorWithMessage(c, fiber.StatusConflict, constants.ErrInvalidAction, err.Error())
			}
...
func HandleFetchPost(s *usecases.PostService) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		if post.PostKind == string(postmodel.PostKindMessage) || (post.ContentableType != nil && *post.ContentableType == string(postmodel.PostKindChat)) {
...
// HandlePostCollectionNotImplemented fails explicitly until likes/bookmarks
// have a privacy-safe read model. Returning the media collection for these
// commands was a silent route-wiring bug and made clients render wrong data as
// a successful response.
func HandlePostCollectionNotImplemented() fiber.Handler {
	return func(c fiber.Ctx) error {
