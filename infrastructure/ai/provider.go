...
func (r *Registry) GenerateText(ctx context.Context, input ports.TextGenerationInput) (ports.TextGenerationResult, error) {
	client, err := r.Resolve(input.Provider)
	if err != nil {
		return ports.TextGenerationResult{}, err
	}

	result, err := client.GenerateText(ctx, GenerateTextInput{
		Prompt:            input.Prompt,
		Model:             input.Model,
	...
	return ports.TextGenerationResult{
