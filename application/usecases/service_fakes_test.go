...
func (r *fakeEngagementRepository) RecordViewOnce(ctx context.Context, engagerID uuid.UUID, engageeID uuid.UUID, kind models.EngagementKind, contentableID uuid.UUID, contentableType models.EngagementContentableType) (bool, error) {
	r.recordedViews = append(r.recordedViews, engagementToggle{
		engagerID: engagerID, engageeID: engageeID, kind: kind, contentableID: contentableID, contentableType: contentableType,
	})
	return r.recordViewResult, r.recordViewErr
}
...
type fakePostRepository struct {
	...
	eventRSVPResult       *postpayloads.EventRSVPResult
	eventRSVPErr          error
...
func (r *fakePostRepository) View(filters types.Filter) (bool, error) {
	r.viewFilter = filters
	return r.viewResult, r.viewErr
}

func (r *fakePostRepository) SetEventRSVP(ctx context.Context, postPublicID int64, userID uuid.UUID, status *postpayloads.EventAttendanceStatus) (*postpayloads.EventRSVPResult, error) {
	r.eventRSVPPostID = postPublicID
	r.eventRSVPUserID = userID
	r.eventRSVPStatus = status
	if r.eventRSVPResult == nil {
		r.eventRSVPResult = &postpayloads.EventRSVPResult{Status: status}
	}
...
type fakeChatRepository struct {
	...
	expiredMessages    []chat.ExpiredMessage
	expireNow          time.Time
	expireLimit        int
	expireErr          error
...
func (r *fakeChatRepository) ExpireMessages(_ context.Context, now time.Time, limit int) ([]chat.ExpiredMessage, error) {
	r.expireNow = now
	r.expireLimit = limit
	return r.expiredMessages, r.expireErr
}

func (r *fakeChatRepository) OpenMessage(_ context.Context, authUser *models.User, chatID, messageID uuid.UUID, now time.Time) (*chat.OpenMessageResult, error) {
